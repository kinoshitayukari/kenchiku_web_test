...
async function populateArticleSidebar() {
  ...
  const statusItem = list.querySelector('[data-article-sidebar-status]');

  const appendIndexLink = () => {
  ...
  try {
    const response = await fetch('/articles/index.html', { credentials: 'same-origin' });
    if (!response.ok) {
    ...
    const filtered = articles.filter((article) => article.absolutePath !== currentPath);
